<template>
    <div class="container">
      ...
      <form @submit.prevent="atualizarLivro" class="content">
  ...
  export default {
    ...
    methods: {
      async buscarLivro() {
        ...
        try {
          const resposta = await new LivroModel().find(livroId);
          this.tituloLivro = resposta.data.titulo;
          this.quantidadeLivro = resposta.data.quantidade;
          this.descricaoLivro = resposta.data.descricao;
          this.assuntoLivro = resposta.data.assunto;
          this.imagemURLLivro = resposta.data.imagemURL;
          this.paginas = resposta.data.paginas;
          this.editoraSelecionada = resposta.data.editoraId;
          this.autoresSelecionados = resposta.data.autores;
          this.categoriasSelecionadas = resposta.data.categorias;
          this.indicadorEstante = resposta.data.indicadorEstante;
          this.estanteSelecionada = resposta.data.estante;
        } catch (error) {
            if (error.response.status === 404) {
              alert('Livro não encontrado.');
              this.$router.push({ name: 'GerenciarLivros' });
            } else {
              console.error(error);
    ...
    async atualizarLivro() {
      ...
      const livroData = {
        ...
        autoresIds: this.autoresSelecionados.map(autor => autor.id),
        categoriasIds: this.categoriasSelecionadas.map(categoria => categoria.id),
        //indicadorEstante: this.indicadorEstante,
        //estanteId: this.estanteSelecionada ? this.estanteSelecionada.id : null,
      };
      try {
        console.log(livroData);
        await new LivroModel().update(livroId,livroData);
